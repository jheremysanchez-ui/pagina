...
class GetItemsView(APIView):
  def get(self, request, format=None):
    ...
    try:
      ...
      return Response(
        {'error': 
          'Algo salió mal al recuperar los artículos del carrito'}, 
...
class GetTotalView(APIView):
  def get(self, request, format=None):
    ...
    try:
      ...
      return Response(
        {'error': 'Algo salió mal al recuperar los costos totales'},
      status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class GetItemTotalView(APIView):
  def get(self, request, format=None):
    ...
    try:
      ...
      return Response(
        {'error': 'Algo salió mal al obtener un número total de artículos'},
      status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class UpdateItemView(APIView):
  def put(self, request, format=None):
    ...
    try:
      ...
      return Response(
        {'error': 'La ID del producto debe ser un entero'},
      status=status.HTTP_404_NOT_FOUND)    
    try:
      count = int(data['count'])
    except:
      return Response(
        {'error': 'El valor de conteo debe ser un entero'},
      status=status.HTTP_404_NOT_FOUND)
    try:
      if not Product.objects.filter(id=product_id).exists():
        return Response(
      ...
      if not CartItem.objects.filter(cart=cart, product=product).exists():
        return Response(
          {'error': 'Este producto no está en su carrito'},
        status=status.HTTP_404_NOT_FOUND)
      ...
      if count <= quantity:
        ...
        return Response(
        {'error': 'No hay suficiente de este artículo en stock'},
        status=status.HTTP_200_OK)
    except:
        return Response(
          {'error': 'Algo salió mal al actualizar el carrito'},
        status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class EmptyCartView(APIView):
  def delete(self, request, format=None):
    ...
    try:
      ...
      return Response(
        {'success': 'Carro vaciado con éxito'},
      status=status.HTTP_200_OK)
    except:
      return Response(
        {'error': 'Algo salió mal vaciando el carrito'},
      status=status.HTTP_500_INTERNAL_SERVER_ERROR)
