...
class GenerateTokenView(APIView):
    def get(self, request, format=None):
        try:
            ...
            return Response(
                {'error': 'Algo salió mal al recuperar el braintree token'},
    ...
    def get(self, request, format=None):
        ...
        try:
            ...
            if not CartItem.objects.filter(cart=cart).exists():
                return Response(
                    {'error': 'Necesita tener artículos en el carrito'},
            ...
            for cart_item in cart_items:
                if not Product.objects.filter(id=cart_item.product.id).exists():
                    return Response(
                        {'error': 'El producto con ID proporcionado no existe'},
                        status=status.HTTP_404_NOT_FOUND
                    )
            ...
            import traceback
            print("ERROR en bloque cupón:")
            traceback.print_exc()
            return Response(
                {'error': 'Algo salió mal al recuperar la información total del pago'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
class ProcessPaymentView(APIView):
    def post(self, request, format=None):
        ...
        for cart_item in cart_items:
            if not Product.objects.filter(id=cart_item.product.id).exists():
                return Response(
                    {'error': 'La transacción falló, no existe un ID de producto'},
                    status=status.HTTP_404_NOT_FOUND
                )
            if int(cart_item.count) > int(cart_item.product.quantity):
                return Response(
        ...
        if newTransaction.is_success or newTransaction.transaction:
            ...
            try:
                ...
                return Response(
                    {'error': 'La transacción fue exitosa pero no se pudo crear el pedido'},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )
            ...
            for cart_item in cart_items:
                try:
                    ...
                    return Response(
                        {'error': 'La transacción fue exitosa y se creó el pedido, pero no se logró crear un artículo de pedido.'},
                        status=status.HTTP_500_INTERNAL_SERVER_ERROR
                    )
            ...
            return Response(
                {'success': 'Transacción exitosa y orden creada'},
                status=status.HTTP_200_OK
            )
        else:
            return Response(
